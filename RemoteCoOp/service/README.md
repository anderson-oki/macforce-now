...
The panel uses a generated self-signed HTTPS certificate unless `MACFORCE_NOW_REMOTE_COOP_PANEL_CERT` and `MACFORCE_NOW_REMOTE_COOP_PANEL_KEY` are configured. Browsers will warn on first access to a self-signed certificate.

## Login

Use a system username and password. Access is allowed for members of `macforce-now-coop-admin`. If that group does not exist, the panel falls back to local administrator groups.

The installers create `macforce-now-coop-admin` and add the invoking user plus the panel service user to it. Override the login account added by the installer with `LOGIN_USER=your-user`.
...
```sh
sudo systemctl status macforce-now-remote-coop-panel
sudo journalctl -u macforce-now-remote-coop-panel -f
sudo systemctl restart macforce-now-remote-coop-panel
```
...
```sh
sudo launchctl print system/com.macforce-now.remote-coop.panel
tail -f /var/log/macforce-now-remote-coop-panel.log
