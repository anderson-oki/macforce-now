    ...
    private enum UpdateError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .invalidResponse(let message), .downloadFailed(let message), .validationFailed(let message), .installerLaunchFailed(let message):
                return message
            case .noReleaseAsset:
                return "The latest GitHub release does not include an OpenNOW macOS zip asset."
            case .notBundledApp:
                return "Updates can only be installed from the packaged OpenNOW.app bundle."
            case .extractionFailed:
                return "The update archive could not be extracted."
            }
        }
    }
